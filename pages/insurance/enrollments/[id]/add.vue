<!--
  Mekari Flex — Insurance · Enrollment · Add employees (enrollment-level)
  Reached from the enrollment detail "Add employee" header action. Unlike the
  plan-level flow, no plan is pre-selected — so each picked employee must be
  assigned an insurance plan here before saving.
  Pixel 3 DT 2.4 only — no <style>, no raw CSS.
-->
<script setup lang="ts">
import {
  Pixel, MpFlex, MpText, MpTextlink, MpButton, MpAvatar, MpSelect,
...
// Plans available per enrollment — drives the per-employee plan picker.
...
const enrollmentPlans = computed(() => ENROLLMENT_PLANS[enrollmentId.value] ?? ENROLLMENT_PLANS['e-2026'])
function planName(id: string) { return enrollmentPlans.value.find((p) => p.id === id)?.name ?? '' }
...
watchEffect(() => {
  setPageHeader?.({
    ...
    breadcrumb: [
      ...
      { label: enrollment.value.name, to: enrollmentDetailPath.value },
    ],
    headerRight: null,
  })
})
...
interface RowConfig { planId: string; effectiveDate: Date; spouse: boolean; children: string[] }
...
// Every added employee must be assigned a plan before saving.
const allHavePlan = computed(() => addedIds.value.every((id) => !!rowConfig[id]?.planId))

// ── Row selection + bulk plan assignment ─────────────────────────────────────
const selected = ref<string[]>([])
const allSelected = computed(() => addedRows.value.length > 0 && addedRows.value.every((r) => selected.value.includes(r.id)))
...
function toggleAll() {
  selected.value = allSelected.value ? [] : addedRows.value.map((r) => r.id)
}
function clearSelection() { selected.value = [] }
// Esc clears the current selection (matches the header hint).
function onKeydown(e: KeyboardEvent) {
  if (e.key === 'Escape' && selected.value.length) clearSelection()
}
onMounted(() => window.addEventListener('keydown', onKeydown))
onUnmounted(() => window.removeEventListener('keydown', onKeydown))
function assignPlanToSelected(planId: string) {
  for (const id of selected.value) {
    if (rowConfig[id]) rowConfig[id].planId = planId
  }
  const n = selected.value.length
  selected.value = []
  toast.notify({ position: 'top-center', variant: 'success', title: `Plan assigned to ${n} ${n === 1 ? 'employee' : 'employees'}` })
}
...
function applyDrawer() {
  ...
  for (const id of drawerSelected.value) {
    if (!rowConfig[id]) rowConfig[id] = { planId: '', effectiveDate: defaultEffectiveDate(), spouse: false, children: [] }
  }
  addedIds.value = [...drawerSelected.value]
  selected.value = selected.value.filter((id) => addedIds.value.includes(id))
...
// ── Footer actions ───────────────────────────────────────────────────────────────
function cancel() { router.push(enrollmentDetailPath.value) }
function save() {
  if (!addedRows.value.length || !allHavePlan.value) return
  toast.notify({ position: 'top-center', variant: 'success', title: 'Employees enrolled' })
  router.push(enrollmentDetailPath.value)
}
...
const planCell = css({ paddingTop: '2', paddingBottom: '2', verticalAlign: 'middle', width: '200px' })
const planMenu = css({ width: '200px', maxWidth: '200px', maxHeight: '280px', overflowY: 'auto' })
// Fixed header height so the row doesn't grow when it switches to the bulk
// action bar (the md "Assign" button is taller than plain header text).
// "Esc" keycap — white key with border + subtle bottom shadow (keyboard look).
const escKey = css({
  display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
  height: '24px', minWidth: '24px', paddingInline: '2',
  borderRadius: 'md', borderWidth: '1px', borderStyle: 'solid', borderColor: 'border.default',
  background: 'background.neutral', color: 'text.secondary',
  fontFamily: 'body', fontSize: 'sm', fontWeight: 'semiBold', lineHeight: 'none',
})
const escKeyShadow = { boxShadow: '0 1.5px 0 0 var(--mp-colors-border-default)' }
const headCell = css({ height: '56px', paddingTop: '0', paddingBottom: '0', verticalAlign: 'middle' })
const bulkHeadCell = css({ height: '56px', paddingTop: '0', paddingBottom: '0', verticalAlign: 'middle' })
const dateCell = css({ paddingTop: '2', paddingBottom: '2', verticalAlign: 'middle', width: '200px' })
const removeBtn = css({
  display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
  ...
  <MpFlex direction="column" gap="6" flex="1" minHeight="0">
    ...
    <MpTableContainer v-if="addedRows.length">
      <MpTable :is-hoverable="false">
        <MpTableHead>
          <!-- Selection mode: the header row becomes the bulk action bar -->
          <MpTableRow v-if="selected.length">
            <MpTableCell as="th" :colspan="5" :class="bulkHeadCell">
              <MpFlex align="center" gap="3" width="100%">
                <MpCheckbox id="select-all-employees" :is-checked="allSelected" :is-indeterminate="someSelected" @change="toggleAll" />
                <MpText size="body" weight="semiBold" color="text.default">{{ selected.length }} selected</MpText>
                <MpPopover id="bulk-assign-plan" is-close-on-select use-portal>
                  <MpPopoverTrigger>
                    <MpButton variant="secondary" size="md">Assign insurance plan</MpButton>
                  </MpPopoverTrigger>
                  <MpPopoverContent :class="planMenu">
                    <MpPopoverList>
                      <MpPopoverListItem v-for="p in enrollmentPlans" :key="p.id" @click="assignPlanToSelected(p.id)">{{ p.name }}</MpPopoverListItem>
                    </MpPopoverList>
                  </MpPopoverContent>
                </MpPopover>
                <MpFlex align="center" gap="2" :class="css({ marginLeft: 'auto' })">
                  <Pixel.kbd :class="escKey" :style="escKeyShadow">Esc</Pixel.kbd>
                  <MpText size="body" color="text.secondary">to clear selection</MpText>
                </MpFlex>
              </MpFlex>
            </MpTableCell>
          </MpTableRow>
          <!-- Default: column headers (select-all checkbox sits inside the first column) -->
          <MpTableRow v-else>
            <MpTableCell as="th" :class="headCell">
              <MpFlex align="center" gap="3">
                <MpCheckbox id="select-all-employees" :is-checked="allSelected" :is-indeterminate="someSelected" @change="toggleAll" />
                <MpText size="label" weight="semiBold" color="text.default">Employee name</MpText>
              </MpFlex>
            </MpTableCell>
            <MpTableCell as="th" :class="headCell">Insurance plan</MpTableCell>
            <MpTableCell as="th" :class="headCell">Effective start date</MpTableCell>
            <MpTableCell as="th" :class="headCell">Dependents</MpTableCell>
            <MpTableCell as="th" :class="[headCell, css({ width: '56px' })]" />
          </MpTableRow>
        </MpTableHead>
        <MpTableBody>
          <MpTableRow v-for="row in addedRows" :key="row.id">
            <MpTableCell as="td" :class="cellPad">
              <MpFlex align="center" gap="3">
                <MpCheckbox :id="`select-${row.id}`" :is-checked="selected.includes(row.id)" @change="toggleRow(row.id)" />
            ...
            <MpTableCell as="td" :class="planCell">
              <MpPopover :id="`plan-select-${row.id}`" is-close-on-select use-portal>
                <MpPopoverTrigger>
                  <MpSelect :model-value="rowConfig[row.id].planId" placeholder="Select plan" size="md" is-full-width @mousedown.prevent>
                    <option v-if="rowConfig[row.id].planId" :value="rowConfig[row.id].planId">{{ planName(rowConfig[row.id].planId) }}</option>
                  </MpSelect>
                </MpPopoverTrigger>
                <MpPopoverContent :class="planMenu">
                  <MpPopoverList>
                    <MpPopoverListItem
                      v-for="p in enrollmentPlans"
                      :key="p.id"
                      :is-active="rowConfig[row.id].planId === p.id"
                      @click="rowConfig[row.id].planId = p.id"
                    >{{ p.name }}</MpPopoverListItem>
                  </MpPopoverList>
                </MpPopoverContent>
              </MpPopover>
            </MpTableCell>
    ...
    <div :class="footer">
      ...
      <MpButton variant="primary" size="md" :is-disabled="!addedRows.length || !allHavePlan" @click="save">Save</MpButton>
    </div>
  </MpFlex>
    ...
    <MpDrawerContent>
      ...
      <MpDrawerBody>
        <MpFlex direction="column" gap="4" :class="css({ height: '100%' })">
          <MpText size="body" color="text.default">Select employees to enroll in this enrollment.</MpText>
